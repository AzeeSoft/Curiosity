Shader "Azee/HologramSeeThroughShader"
{
	Properties
	{
		// General
		...
		[MaterialToggle] _UseLocalDirection ("Use Local Direction", Int) = 0
		...
		_RimPower ("Rim Power", Range(0.0, 10)) = 5.0
		...
		[MaterialToggle] _GlowAnimate ("Glow Animate", Int) = 1
		...
		// Clip
		[HideInInspector] _IsClipping ("Is Clipping", Int) = 0
		[HideInInspector] _ClipWorldCenter("Clip World Center", Vector) = (0, 0, 0, 0)
		[HideInInspector] _ClipRadius ("Clip Radius", Float) = 0
		[HideInInspector] _ClipSmoothing ("Clip Smoothing", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				float clipValue = 1;
				if (_IsClipping == 1) {
					float clipDist = distance(i.worldVertex, _ClipWorldCenter);

					if (clipDist > _ClipRadius) {
						clipValue = 1 - saturate((_ClipRadius - clipDist) / _ClipSmoothing);
						if (clipValue == 0) {
							clip (-1);
						}
					}
				...
				half dirVertex;
				if (_UseLocalDirection == 1) {
					dirVertex = (dot(i.localVertex, normalize(float4(_Direction.xyz, 1.0))) + 1) / 2;
				} else {
					dirVertex = (dot(i.worldVertex, normalize(float4(_Direction.xyz, 1.0))) + 1) / 2;
				}
				...
				#ifdef _GLOW_ON
				if (_GlowAnimate == 1) {
					glow = frac(dirVertex * _GlowTiling - _Time.x * _GlowSpeed) * _GlowIntensity;
				} else {
					glow = _GlowIntensity;
				}
				#endif
				...
				col.a = texColor.a * _Alpha * (scan + rim + glow) * flicker * clipValue;
